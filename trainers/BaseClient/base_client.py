...
class BaseClientTrainer(ClientTrainer, ABC):
    ...
    def _calculate_model_computation(self):
        ...
        train_loader = self._get_dataloader(
            dataset=self.train_dataset, client_id=dummy_idx
        )
        for step, batch in enumerate(train_loader):
            ...
            self.logger.debug(
                f"Model Type: {self.model_config.model_type}, "
    ...
    def _train_alone(self, idx: int, model_parameters: torch.Tensor, *args, **kwargs):
        ...
        self.logger.info(f"local step of client {idx} is {len(train_loader)}")
    ...
    def _train_alone_with_steps(
        self,
        idx: int,
        client_data_indexes: dict,
        step: int,
        ...
        **kwargs,
        ...
        for i in range(step):
            ...
            self._on_epoch_with_a_batch(batch, optimizer, scheduler)
            if self.federated_config.pson and self.stop_early:
                self.logger.critical(f"local stop early in {epoch}")
                break
        self._on_epoch_end(idx)

    def _train_alone_with_position(
        self, idx: int, model_parameters: torch.Tensor, position="all", *args, **kwargs
    ):
        """local training for Client"""
    ...
    def _train_alone_with_range(
        ...
        **kwargs,
        ...
        if model_parameters is not None:
            SerializationTool.deserialize_model(self._model, model_parameters)
        #########################################################
        group2depth = {7: 3, 1: 4, 9: 5, 8: 6, 5: 7, 6: 8, 3: 9, 10: 10, 2: 11, 4: 12}
        group2depth2 = {7: 12, 1: 11, 9: 10, 8: 9, 5: 8, 6: 7, 3: 6, 10: 5, 2: 4, 4: 3}
        group2depth3 = {7: 7, 1: 7, 9: 7, 8: 7, 5: 7, 6: 7, 3: 7, 10: 7, 2: 7, 4: 7}
    ...
    def local_process(
        self, id_list: List, payload: List, step=None, client_data_indexes=None
    ):
        """local process for Federated Learning"""
        model_parameters = payload[0]
        self.param_list = self.fed_train_with_steps(
            model_parameters, id_list, client_data_indexes, step
        )
    ...
    def fed_train(self, model_parameters: torch.Tensor, id_list: List):
        ...
        for idx in id_list:
            self._train_alone(idx=idx, model_parameters=model_parameters)
    ...
    def fed_train_with_group_aggregation(
        self, model_parameters: torch.Tensor, id_list: List
    ):
        param_list = []
    ...
    def fed_train_with_steps(
        ...
        param_list = []
        for idx in id_list:
            self._train_alone_with_steps(
                idx=idx,
                client_data_indexes=client_data_indexes,
                step=step,
                model_parameters=model_parameters,
            )
            param_list.append(self.model_parameters)
    ...
    def _build_optimizer(self, model, train_dl_len):
        if self.training_config.max_steps > 0:
            ...
            self.training_config.num_train_epochs = (
                ...
                // (train_dl_len // self.training_config.gradient_accumulation_steps)
                + 1
            )
        else:
            t_total = (
    ...
    def _mixed_train_model(self, model, optimizer):
        if self.training_config.fp16:
            ...
            model, optimizer = amp.initialize(
                model, optimizer, opt_level=self.training_config.fp16_opt_level
            )

            # multi-gpu training (should be after apex fp16 initialization)
        if self.training_config.n_gpu > 1:
            self.logger.warning(
                "We haven't tested our model under multi-gpu. Please be aware!"
            )
    ...
    def _on_epoch(self, train_loader, optimizer, scheduler):
        for step, batch in enumerate(train_loader):
            ...
            label = inputs["labels"]
            if (
                self.model_config.model_type != "distilbert"
                or self.model_config.model_type != "roberta"
            ):
            ...
            if (step + 1) % self.training_config.gradient_accumulation_steps == 0:
                if self.training_config.fp16:
                    torch.nn.utils.clip_grad_norm_(
                        amp.master_params(optimizer), self.training_config.max_grad_norm
                    )
                else:
                    torch.nn.utils.clip_grad_norm_(
                        self._model.parameters(), self.training_config.max_grad_norm
                    )
    ...
    def _on_epoch_with_a_batch(self, batch, optimizer, scheduler):
        self._model.train()
        batch = tuple(t.to(self.device) for t in batch)
        inputs = {
            "input_ids": batch[0],
    ...
    def main_loop(self):
        ...
        while True:
            ...
            if message_code == MessageCode.Exit:
                ...
                id_list_and_step, payload = (
                    payload[0].to(torch.int32).tolist(),
                    payload[1:],
                )
                id_list = id_list_and_step[:-1]
                ...
                if self._trainer.type == SERIAL_TRAINER:  # serial
                    self._trainer.local_process(
                        ...
                        payload=payload,
                        step=step,
                        client_data_indexes=client_idx_dict,
                    )
                ...
                self.synchronize()
                # Update local training indexes of workers
                for client_idx in client_idx_dict.keys():
                    client_idx_dict[client_idx] = (client_idx_dict[client_idx] + step) % all_local_step
            else:
                raise ValueError(
                    f"Invalid MessageCode {message_code}. Please check MessageCode list."
                )
